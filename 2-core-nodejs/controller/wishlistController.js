const ask = require("../helpers/askQuestion.js");
...
async function handleAddItem() {
  console.log(
    `${cyan}${bold}--------------- Adding an Item -----------------${reset}\n`,
  );
  const name = await ask("Type the name: ");
  ...
  const store = await ask("Type the store: ");
...
async function handleEditItem() {
  console.log(
    `${cyan}${bold}--------------- Editing an Item -----------------${reset}\n`,
  );
  const data = await viewItems();
  console.table(data);
...
async function handleDeleteItem() {
  console.log(
    `${cyan}${bold}--------------- Deleting an Item -----------------${reset}\n`,
  );
  const data = await viewItems();
  console.table(data);

  const id = await ask("Type the id to delete: ");
...
async function run() {
  ...
  do {
    ...
    switch (option) {
      ...
      case "6":
        console.log(
          `${cyan}${bold}--------------- WISHLIST SUMMARY -----------------${reset}\n`,
        );
        await summary();
        break;
